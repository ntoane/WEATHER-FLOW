...
from Models.CLCRN.manifold.sphere import Sphere
from Utils.CLCRN_Utils.createData import NormScaler
...
def load_dataset(dataset_dir, split, batch_size, position_file, val_batch_size=None, include_context=False, **kwargs):
    ...
    train_set = Dataset(
        dataset_dir, split, mode='trn', batch_size=batch_size, include_context=include_context
    )

    validation_set = Dataset(
        dataset_dir, split, mode='val', batch_size=val_batch_size, include_context=include_context
    )

    test_set = Dataset(
        dataset_dir, split, mode='test', batch_size=val_batch_size, include_context=include_context
    )
...
class KernelGenerator:
    def __init__(self, lonlat, k_neighbors=25, local_map='fast') -> None:
        ...
        matrix, self.ratio_lists = self.XY2Ratio(self.MLP_inputs[:,-2:], k_neighbors=self.k_neighbors)
...
class Dataset(Dataset):
    ...
    def __init__(self, dataset_dir,split, mode, batch_size, scaler=None, include_context=True):
        ...
        with open(dataset_dir + '/{}_split_{}.pkl'.format(mode,split), 'rb') as f:
            data = pickle.load(f)
        self.x = data['x']
        self.y = data['y']
